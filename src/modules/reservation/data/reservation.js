...
export async function submit({reservation, loading, close, notify}) {
  const r = normalize(reservation)
  loading(true)
  const {roomId, from, to} = r
  try {
    const available = await isAvailable(roomId, {start: from, end: to})
    if (available === true) {
      loading(false)
      close()
    ...
    loading(false)
    notify("reservation.submit-error", {type: "error", message: error.message})
    return false
  }
}
 ...
 */
export function normalize(reservation) {
